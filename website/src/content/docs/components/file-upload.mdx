...
metaDescription: File Upload component for React with drag-and-drop support and file validation. Enable users to select and upload files with progress tracking.
category: forms
links:
...
```tsx
import { FileUpload } from '@zalify-ui/react'
```
