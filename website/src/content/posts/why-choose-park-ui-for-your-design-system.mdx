---
title: Why Choose ZUI for Your Design System?
description: "Build better design systems with ZUI: complete code ownership, beautiful components, and AI-ready architecture for React and Solid.js."
authors: ["chrizzly"]
...
ZUI flips the script entirely by giving you the source code directly - offering complete control without sacrificing beauty or accessibility.

## The Open Code Philosophy

Unlike traditional component libraries, ZUI distributes components as source code, not NPM packages. This simple shift changes everything.

Every component lives directly in your codebase. You get full visibility into implementation and styling with no black boxes or abstracted APIs. Need a new button variant? Edit the recipe directly. No more CSS overrides or complex theming APIs. Changes that would take hours in traditional libraries take minutes with ZUI.
...
ZUI isn't built from scratch. It's built on proven technologies: Ark UI and Panda CSS.
...
The result? You get accessibility and interaction logic from Ark UI, beautiful customizable design from ZUI recipes, and complete control since everything lives in your codebase.

## Beautiful by Default, Customizable by Design

Developed in collaboration with Brains & Pixels, ZUI provides a systematic design approach that looks professional out of the box.
...
ZUI also includes thoughtful design tokens like nested border radius values (l1, l2, l3) and adaptive shadows that work in both light and dark modes. There's even Figma integration with a free Foundations Kit and Pro Kit for seamless designer-to-developer workflow.

## One API, Multiple Frameworks

ZUI supports React, and Solid.js with a consistent API across all three frameworks. This means you learn once and use everywhere. You can switch frameworks without relearning component APIs, share design systems across different projects, and future-proof against framework trends.

Getting components into your project is simple. Use the CLI with `npx @zuish/cli add button` or browse the documentation and copy source code directly. No bloated dependencies for components you'll never use.

## Developer Experience That Scales

ZUI integrates seamlessly with existing Panda CSS projects. Initialize with `npx @zuish/cli init` and add components as needed. Because the code lives in your repository, you never have to worry about dependency updates breaking your UI or fighting with library APIs during upgrades.
...
## When Should You Use ZUI?

ZUI is perfect for projects requiring heavy customization, teams building custom design systems, and startups that need to move fast while owning their code. If accessibility is non-negotiable or you're embracing AI-assisted development, ZUI is an excellent choice.
...
Getting started with ZUI takes just minutes. You'll need an existing Panda CSS setup, then install the Ark UI and Lucide Icons dependencies. Initialize with `npx @zuish/cli init`, add your first component with `npx @zuish/cli add button`, and start customizing by editing component recipes to match your brand.
...
ZUI represents a paradigm shift in how we think about component libraries. Traditional libraries optimize for getting started quickly, but they become obstacles as your needs evolve. ZUI optimizes for long-term success by giving you ownership from day one.
