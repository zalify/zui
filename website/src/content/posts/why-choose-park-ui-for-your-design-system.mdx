...
Getting components into your project is simple. Use the CLI with `npx @zui/cli add button` or browse the documentation and copy source code directly. No bloated dependencies for components you'll never use.

## Developer Experience That Scales

ZUI integrates seamlessly with existing Panda CSS projects. Initialize with `npx @zui/cli init` and add components as needed. Because the code lives in your repository, you never have to worry about dependency updates breaking your UI or fighting with library APIs during upgrades.
...
Getting started with ZUI takes just minutes. You'll need an existing Panda CSS setup, then install the Ark UI and Lucide Icons dependencies. Initialize with `npx @zui/cli init`, add your first component with `npx @zui/cli add button`, and start customizing by editing component recipes to match your brand.
